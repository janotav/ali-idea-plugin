...
public class ExpressionResolverTest extends IntellijTest {
    ...
    public void testResolveDisplayValueSync() {
        ExpressionResolver resolver = new ExpressionResolver(new TranslatorSync());
        ...
        String value = resolver.resolveDisplayValue("A OR B", new ValueCallback() {
            ...
            public void value(final String value) {
                handler.done(new Runnable() {
                    @Override
                    public void run() {
                        Assert.assertEquals("(a OR b)", value);
                    }
                });
            }
        });
    ...
    public void testToRESTQuery() {
        ExpressionResolver resolver = new ExpressionResolver(new TranslatorSync());
        Assert.assertEquals("A OR B", resolver.toRESTQuery("A OR B"));
    }

    @Test
    public void testGetTranslator() {
        TranslatorSync translator = new TranslatorSync();
        ExpressionResolver resolver = new ExpressionResolver(translator);
        Assert.assertEquals(translator, resolver.getTranslator());
    }
}
