...
public class TestAcceptedAction extends EntityAction {

    public TestAcceptedAction() {
        super("Passed", "Acceptance Test Passed", IconLoader.getIcon("/debugger/threadStates/idle.png"));
    }
    ...
    protected void update(AnActionEvent event, Project project, Entity entity) {
        boolean passed = "Passed".equals(entity.getPropertyValue("status"));
        event.getPresentation().setEnabled(!passed);
    }

    @Override
    protected void actionPerformed(AnActionEvent event, Project project, Entity entity) {
        entity.setProperty("status", "Passed");
